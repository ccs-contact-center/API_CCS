...
userExist = async (user) => {
  try {
    ...
    let result = await pool
      ...
      .input("ID_CCS", user)
      .query(
        "SELECT COUNT(*) as Exist FROM CCS.dbo.SYS_Usuarios WHERE id_ccs = @ID_CCS"
      );
    if (result[0].Exist == 0) {
      return false;
    } else {
...
nameExist = async (nombre, paterno, materno, fnac) => {
  try {
    ...
    let result = await pool
      ...
      .query(
        "SELECT COUNT(*) as Exist FROM CCS.dbo.SYS_Usuarios WHERE nombres + ' ' + paterno + ' ' + materno + ' '+ CONVERT(NVARCHAR(10),fecha_nacimiento,101) = @ID_CCS"
      );
    if (result[0].Exist == 0) {
      return false;
    } else {
...
router.get("/Candidatos", (req, res) => {
  ...
  const queryString = `SELECT id, CONVERT(NVARCHAR(10),fecha_reclutamiento,103) as Fecha_Reclutamiento, nombres + ' '+ paterno + ' ' + materno as Candidato, CONVERT(NVARCHAR(10),fecha_nacimiento,103) as Fecha_Nacimiento, DATEDIFF([YEAR],fecha_nacimiento,GETDATE()) as Edad, edo_civil, tel_cel , tel_casa , email,campania, turno FROM CCS.dbo.SYS_Candidatos WHERE status IN (${generateString})`;
...
router.get("/CandidatoValidacion", (req, res) => {
  //Valida si existe un id en formato 1-4-3
  sql.connect(constants.dbCluster, (err) => {
    if (err) console.log(err);
...
router.get("/Usuarios", (req, res) => {
  ...
  const queryString = `SELECT * FROM CCS.dbo.SYS_Usuarios WHERE status IN (${generateString})`;
...
router.post("/Usuario", async (req, res) => {
  ...
  if (nombreusuario === true && idusuario === true) {
    ...
    sql.connect(constants.dbCluster, (err) => {
      ...
      request.query(
        `INSERT INTO CCS.dbo.SYS_Usuarios (
           su
          ,empleado_sim
          ,id_ccs
          ,pass_ccs
